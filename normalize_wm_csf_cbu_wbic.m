    ...
    for s = 1:numel(subdir)
        ...
        for q = 1:numel(subsubdir)
            ...
            if ~isempty(strmepi) && t1_flag
                ...
                matlabbatch{1}.spm.spatial.normalise.write.subj.resample = {[fullfile(subfulldirs,subsubdir(q).name),'/c2T1w.nii']};
                matlabbatch{2}.spm.spatial.normalise.write.subj.def = {forwd};
                matlabbatch{2}.spm.spatial.normalise.write.subj.resample = {[fullfile(subfulldirs,subsubdir(q).name),'/c3T1w.nii']};
                t1_flag = 0;
            end 
        end
        try
